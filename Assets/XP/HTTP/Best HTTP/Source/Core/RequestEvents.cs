        ...
        internal static void HandleRequestStateChange(RequestEventInfo @event)
        {
            ...
            switch (@event.State)
            {
                ...
                case HTTPRequestStates.Finished:
                    ...
                    {
                        HTTPManager.Logger.Exception("RequestEventHelper", string.Format("处理请求状态改变 - Cache probe - CurrentUri: \"{0}\" State: {1} StatusCode: {2}", source.CurrentUri, source.State, source.Response != null ? source.Response.StatusCode : 0), ex, source.Context);
                    }
#endif

                    if (source.Callback != null)
                    {
                        //UnityEngine.Debug.Log(source.Response.DataAsText);
                        try
                        {
                        ...
                        {
                            HTTPManager.Logger.Exception("RequestEventHelper", "处理请求状态改变 " + @event.State, ex, source.Context);
                        }
                    }
